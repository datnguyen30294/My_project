...
for var in DOCKERHUB_USERNAME DOCKERHUB_TOKEN REGISTRY_IMAGE APP_KEY DB_PASSWORD \
           ...
           NUXT_PUBLIC_API_URL NUXT_PUBLIC_PLATFORM_DOMAIN NUXT_PUBLIC_BASE_DOMAIN NUXT_API_INTERNAL_URL \
           MAIL_USERNAME MAIL_PASSWORD RESI_MART_INTERNAL_TOKEN RESIMART_DB_PASSWORD; do
  if [ -z "${!var:-}" ]; then
...
# --- Services (NodePort; routed via NPM on host) ---
echo ""
...
echo ""
echo "--- Deploying backend (${REGISTRY_IMAGE}:backend-latest) ---"
...
echo ""
echo "--- Deploying frontend (${REGISTRY_IMAGE}:frontend-latest) ---"
