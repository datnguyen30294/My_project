...
function isActive(to?: string): boolean {
  ...
  // Only match startsWith if no other sibling route is a more specific match
  return route.path.startsWith(to + '/') && !navigationItems.value.some(item =>
    item.children?.some(child => child.to !== to && child.to.startsWith(to + '/') && route.path.startsWith(child.to))
  )
}

function isGroupActive(item: ReturnType<typeof useNavigation>['navigationItems']['value'][number]): boolean {
