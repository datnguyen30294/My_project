...
class OgTicketExternalService implements OgTicketExternalServiceInterface
{
    ...
    public function getOrderInfoByTicketId(int $ticketId): ?array
    {
        ...
        return $tenant->run(function () use ($ticketId): ?array {
            ...
            $quote = app(QuoteRepository::class)->findResidentVisibleByOgTicket($ogTicket->id);

            if (! $quote) {
                return null;
            }

            $quote->load('order.lines');
            $order = $quote->order;
    ...
    public function getQuoteInfoByTicketId(int $ticketId): ?array
    {
        ...
        return $tenant->run(function () use ($ticketId): ?array {
            ...
            // Repository chỉ trả quote ở trạng thái cư dân được thấy
            // (ManagerApproved/Approved/ResidentRejected), deterministic khi
            // có nhiều quote active (luồng báo giá lại).
            $quote = app(QuoteRepository::class)->findResidentVisibleByOgTicket($ogTicket->id);
    ...
    public function decideQuoteByTicketId(int $ticketId, string $action, ?string $reason): void
    {
        ...
        $tenant->run(function () use ($ticketId, $action, $reason): void {
            ...
            // Cùng logic chọn quote với getQuoteInfoByTicketId để cư dân
            // quyết định đúng báo giá đang hiển thị.
            $quote = app(QuoteRepository::class)->findResidentVisibleByOgTicket($ogTicket->id);
