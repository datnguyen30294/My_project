...
class SaveCommissionConfigRequest extends BaseFormRequest
{
    ...
    /**
     * Validate party rules percent sum to 100 (including platform fee %).
     *
     * Platform % phải lấy CÙNG nguồn với lúc hiển thị (cấu hình phí tenant qua
     * ExternalService) để tổng hợp lệ trên UI cũng hợp lệ khi lưu.
     */
    private function validatePartyPercentSum(Validator $validator): void
    {
        $feePolicy = app(TenantConfigExternalServiceInterface::class)->getFeePolicyForCurrentTenant();
        $sum = $feePolicy?->appliedPercent() ?? 0.0;
