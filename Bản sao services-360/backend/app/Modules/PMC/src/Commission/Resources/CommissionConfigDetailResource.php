...
{
    /**
     * @param  array{percent: float, value_fixed: float, source: string}  $platform
     */
    public function __construct(
        private Project $project,
        private ?ProjectCommissionConfig $config,
        private ?\Illuminate\Database\Eloquent\Collection $adjusters = null,
        private array $platform = ['percent' => 0.0, 'value_fixed' => 0.0, 'source' => 'fallback'],
    ) {
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'platform' => [
                'percent' => (float) $this->platform['percent'],
                'value_fixed' => (float) $this->platform['value_fixed'],
                'source' => $this->platform['source'],
            ],
