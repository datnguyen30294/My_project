...
class CommissionConfigRepository extends BaseRepository
{
    ...
    /**
     * Bulk-update the `percent` of party rules keyed by id. Used when rebalancing
     * the parties to a new platform fee — other fields are left untouched.
     *
     * @param  array<int, float>  $idToPercent
     */
    public function updatePartyRulePercents(array $idToPercent): void
    {
        foreach ($idToPercent as $id => $percent) {
            CommissionPartyRule::query()->where('id', $id)->update(['percent' => $percent]);
        }
    }
    ...
    public function getTotalFixedByProject(int $projectId): float
    {
        ...
        $platformFixed = $this->tenantConfig->getFeePolicyForProject($projectId)?->appliedFixed() ?? 0.0;
