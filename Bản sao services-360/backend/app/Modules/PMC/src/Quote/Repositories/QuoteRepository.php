...
class QuoteRepository extends BaseRepository
{
    ...
    /**
     * Find the latest active quote in a resident-visible status for an OgTicket.
     * Resident-visible = ManagerApproved (chờ cư dân duyệt), Approved, ResidentRejected.
     * Deterministic khi có nhiều quote active (effective + draft replacement):
     * draft/sent không bao giờ được trả về cho cư dân.
     */
    public function findResidentVisibleByOgTicket(int $ogTicketId): ?Quote
    {
        /** @var Quote|null */
        return $this->newQuery()
            ->where('og_ticket_id', $ogTicketId)
            ->where('is_active', true)
            ->whereIn('status', [
                QuoteStatus::ManagerApproved->value,
