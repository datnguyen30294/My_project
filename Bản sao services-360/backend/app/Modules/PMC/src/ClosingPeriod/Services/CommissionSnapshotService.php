...
class CommissionSnapshotService implements CommissionSnapshotServiceInterface
{
    ...
    private function calculateFromOverrides(ClosingPeriod $period, Order $order): array
    {
        ...
        // Platform amount calculated separately, using the fee policy applied to
        // the order's project (per-project override → fallback tenant default).
        $projectId = $order->quote?->ogTicket?->project_id;
        $feePolicy = $this->resolveFeePolicy($projectId);
        $platformAmount = $this->calculatePlatformAmount($commissionableTotal, $projectId);
        $snapshots[] = $this->buildSnapshotData(
            ...
            percent: $feePolicy?->appliedPercent() ?? 0.0,
            valueFixed: $feePolicy?->appliedFixed() ?? 0.0,
    ...
    private function calculateFromConfig(ClosingPeriod $period, Order $order): array
    {
        ...
        $partyRecipients = $this->buildPartyRecipients($config, $commissionableTotal, $projectId);
     ...
     * @return list<array<string, mixed>>
     */
    private function buildPartyRecipients(?ProjectCommissionConfig $config, float $pool, ?int $projectId): array
    {
        $recipients = [];

        // Platform (always first, sort_order = 1) — rate from the fee policy applied
        // to this project (per-project override → fallback tenant default).
        $feePolicy = $this->resolveFeePolicy($projectId);
        $recipients[] = [
            'sort_order' => 1,
            ...
            'percent' => $feePolicy?->appliedPercent() ?? 0.0,
            'value_fixed' => $feePolicy?->appliedFixed() ?? 0.0,
        ];
     ...
     */
    public function getCommissionableTotal(Order $order): float
    {
    ...
    /**
     * Platform amount = phí nền tảng áp dụng cho dự án (override theo dự án nếu có,
     * nếu không thì mặc định tenant; fixed-first, % trên phần còn lại).
     * Không có cấu hình phí → 0.
     */
    private function calculatePlatformAmount(float $commissionableTotal, ?int $projectId): float
    {
        return $this->resolveFeePolicy($projectId)?->computeForOrder($commissionableTotal) ?? 0.0;
    }

    /**
     * Phí nền tảng áp dụng cho một đơn: override theo dự án nếu có (và không kế
     * thừa), nếu không thì mặc định tenant. Đơn không gắn dự án → mặc định tenant.
     */
    private function resolveFeePolicy(?int $projectId): ?TenantFeePolicy
    {
        return $projectId !== null
            ? $this->tenantConfig->getFeePolicyForProject($projectId)
