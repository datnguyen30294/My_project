...
class ClosingPeriodService extends BaseService implements ClosingPeriodServiceInterface
{
    ...
    public function getEligibleOrders(int $periodId): Collection
    {
        ...
        $orders = Order::query()
            ...
            ->with([
                ...
                'lines:id,order_id,line_type,line_amount',
            ])
            ->select(['id', 'code', 'total_amount', 'quote_id'])
            ->get();

        // Attach the projected platform fee using the policy effective for each
        // order's project (override → tenant default) so the preview matches what
        // will be frozen. Base = commissionable total (đồng bộ với phần Platform
        // khi đóng băng), KHÔNG phải receivable.
        return $orders->each(function (Order $order): void {
            $base = $this->snapshotService->getCommissionableTotal($order);
            $order->setAttribute('estimated_platform_fee', $this->resolveFeePolicy($order)?->computeForOrder($base) ?? 0);
        });
    }
    ...
    public function addOrders(int $periodId, array $orderIds): ClosingPeriod
    {
        ...
        // Batch-load all orders with relations to avoid N+1. `quote.ogTicket`
        // is needed to resolve the per-project platform fee policy.
        $orders = Order::query()
            ->whereIn('id', $orderIds)
            ->with([
                'receivable',
                'commissionOverrides.account:id,name',
                'lines',
                'quote:id,og_ticket_id',
                'quote.ogTicket:id,project_id',
            ])
            ->get()
            ->keyBy('id');
        ...
        return $this->executeInTransaction(function () use ($period, $orderIds, $orders): ClosingPeriod {
            foreach ($orderIds as $orderId) {
                ...
                // Create pivot record. Phí nền tảng = đúng phần "Platform" trong snapshot
                // (không tính riêng) → là dòng con của tổng hoa hồng, không trừ trùng.
                $this->repository->createPeriodOrder(array_merge([
                ...
                ], $this->platformFeeColumns($this->resolveFeePolicy($order), $this->extractPlatformFee($snapshots))));
            }
    ...
    public function reopen(int $periodId, array $data): ClosingPeriod
    {
        ...
        return $this->executeInTransaction(function () use ($period, $data): ClosingPeriod {
            ...
            foreach ($period->orders as $periodOrder) {
                ...
                // Recompute platform fee with the latest policy effective for the
                // order's project (mirrors how commission is recalculated on reopen
                // — NOT frozen by config version).
                $periodOrder->update(array_merge(
                    ['frozen_commission_total' => round($commissionTotal, 2)],
                    $this->platformFeeColumns($this->resolveFeePolicy($order), $this->extractPlatformFee($snapshots)),
                ));
            }
    ...
    /**
     * Policy effective for an order's project: per-project override when present,
     * otherwise the tenant default. Orders that cannot resolve a project fall back
     * to the tenant default policy.
     */
    private function resolveFeePolicy(Order $order): ?TenantFeePolicy
    {
        $projectId = $order->quote?->ogTicket?->project_id;

        return $projectId !== null
            ? $this->tenantConfig->getFeePolicyForProject((int) $projectId)
            : $this->tenantConfig->getFeePolicyForCurrentTenant();
    }

    /**
     * Build the frozen platform-fee columns for a ClosingPeriodOrder. The amount
     * is the actual "Platform" recipient amount taken from the commission snapshot
     * (single source of truth) — the mode/fixed/percent snapshot the rate that
     * produced it. Null policy (no tenant context / no config) → no fee (0).
     *
     * @return array<string, mixed>
     */
    private function platformFeeColumns(?TenantFeePolicy $feePolicy, float $platformAmount): array
    {
        return [
            'platform_fee_mode' => $feePolicy?->modeValue(),
            'platform_fee_fixed' => $feePolicy?->appliedFixed() ?? 0,
            'platform_fee_percent' => $feePolicy?->appliedPercent() ?? 0,
            'frozen_platform_fee' => round($platformAmount, 2),
        ];
    }

    /**
     * Extract the "Platform" recipient amount from a list of snapshots (0 if absent).
     *
     * @param  list<OrderCommissionSnapshot>  $snapshots
     */
    private function extractPlatformFee(array $snapshots): float
    {
        foreach ($snapshots as $snapshot) {
            if ($snapshot->recipient_type === SnapshotRecipientType::Platform) {
