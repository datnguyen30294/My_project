...
class ScheduleSlotService extends BaseService implements ScheduleSlotServiceInterface
{
    ...
    public function getPersonal(int $accountId, string $month): array
    {
        ...
        $unscheduled = $this->derivation->deriveUnscheduled([$accountId], $from, $to, $shifts->values());
        $unscheduledForAccount = $unscheduled[$accountId] ?? [];
        ...
        return [
            ...
            'unscheduled_cards' => empty($unscheduledForAccount) ? new \stdClass : $unscheduledForAccount,
        ];
    }
    ...
    public function getTeam(string $month, ?int $projectId, ?array $accountIds, bool $strictProject = false): array
    {
        ...
        $unscheduled = ($strictProject && $projectId !== null)
            ? []
            : $this->derivation->deriveUnscheduled($resolvedAccountIds, $from, $to, $shifts->values());
        ...
        $dayCardsByAccount = [];
        $unscheduledByAccount = [];
        foreach ($resolvedAccountIds as $aid) {
            $wsByDate = $wsByAccountDateShift[$aid] ?? [];
            $slotsForAccount = $ticketSlots[$aid] ?? [];
            $unscheduledForAccount = $unscheduled[$aid] ?? [];
            if ($unscheduledForAccount !== []) {
                $unscheduledByAccount[$aid] = $unscheduledForAccount;
            }
            $dayCards = [];
            foreach ($days as $day) {
                $dateKey = $day['date'];
        ...
        return [
            ...
            'unscheduled_cards_by_account' => empty($unscheduledByAccount) ? new \stdClass : $unscheduledByAccount,
        ];
    }
