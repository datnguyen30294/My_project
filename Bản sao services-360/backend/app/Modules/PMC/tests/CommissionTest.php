...
class CommissionTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // Baseline platform fee (5% + 1000đ/đơn) — the commission "Platform" row reads
        // this from the tenant fee policy. Tests have no real tenant, so bind a fake one.
        $this->setFeePolicy(TenantFeeMode::Both, fixed: 1000, percent: 5);
    }
    ...
    public function test_can_show_config_with_all_relations(): void
    {
        ...
        $response->assertStatus(200)
            ...
            ->assertJsonPath('data.platform.source', 'api');
    ...
    public function test_show_config_platform_reads_from_tenant_fee_policy(): void
    {
        $this->setFeePolicy(TenantFeeMode::Both, fixed: 20000, percent: 10);

        $response = $this->getJson("{$this->baseUrl}/projects/{$this->project->id}");

        $response->assertStatus(200)
            ->assertJsonPath('data.platform.percent', 10)
            ->assertJsonPath('data.platform.value_fixed', 20000)
            ->assertJsonPath('data.platform.source', 'api');
    }

    public function test_show_config_platform_falls_back_when_no_fee_policy(): void
    {
        // Không có tenant context / tenant chưa cấu hình phí → fallback config (0).
        $this->setFeePolicy(null);

        $response = $this->getJson("{$this->baseUrl}/projects/{$this->project->id}");

        $response->assertStatus(200)
            ->assertJsonPath('data.platform.percent', 0)
            ->assertJsonPath('data.platform.value_fixed', 0)
            ->assertJsonPath('data.platform.source', 'fallback');
    }

    public function test_show_config_platform_respects_fee_mode(): void
    {
        // PercentPerOrder → chỉ áp % phí, không áp phí cố định dù có cấu hình.
        $this->setFeePolicy(TenantFeeMode::PercentPerOrder, fixed: 20000, percent: 10);
    ...
    public function test_show_config_platform_uses_project_fee_override(): void
    {
        ...
        // Phải hiện override theo dự án (6.000/0), KHÔNG phải mặc định tenant (20.000/10).
        $response->assertStatus(200)
            ->assertJsonPath('data.platform.value_fixed', 6000)
            ->assertJsonPath('data.platform.percent', 0)
            ->assertJsonPath('data.platform.source', 'api');
    }

    /**
     * Bind a fake tenant fee policy so the commission-config "Platform" row can be
     * resolved without a real tenant context (tests run on SQLite).
     */
     ...
     * tenant default, proving the commission-config "Platform" row resolves the
