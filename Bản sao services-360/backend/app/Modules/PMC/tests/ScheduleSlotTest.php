...
class ScheduleSlotTest extends TestCase
{
    ...
    private function createUnscheduledTicket(Account $assignee, string $status = 'in_progress', ?string $assignedAt = null, ?string $completedAt = null): OgTicket
    {
        $attrs = [
            'ticket_id' => 2,
            'requester_name' => 'Resident',
            'requester_phone' => '0900000001',
            'project_id' => null,
            'subject' => 'Dịch vụ sơn tường',
            'channel' => TicketChannel::App->value,
            'status' => $status,
            'priority' => 'high',
            'received_at' => '2026-03-01 08:00:00',
    ...
    public function test_personal_lists_unscheduled_ticket_without_project(): void
    {
        $ticket = $this->createUnscheduledTicket(
            $this->employee,
            status: 'in_progress',
            assignedAt: '2026-04-05 09:00:00',
            completedAt: null,
        ...
        $response->assertStatus(200)
            ->assertJsonPath('data.unscheduled_cards.2026-04-05.0.id', $ticket->id)
            ->assertJsonPath('data.unscheduled_cards.2026-04-05.0.subject', 'Dịch vụ sơn tường')
            ->assertJsonPath('data.unscheduled_cards.2026-04-05.0.priority.value', 'high');

        $this->assertEmpty($response->json('data.day_cards.2026-04-05'));

        $cards0408 = collect($response->json('data.unscheduled_cards.2026-04-08'));
        $this->assertNotNull($cards0408->firstWhere('id', $ticket->id));
    }

    public function test_personal_stops_unscheduled_after_completed(): void
    {
        $ticket = $this->createUnscheduledTicket(
            $this->employee,
            status: 'completed',
            assignedAt: '2026-04-05 09:00:00',
            completedAt: '2026-04-06 16:00:00',
        );
        ...
        $response->assertStatus(200);
        $this->assertNotNull(collect($response->json('data.unscheduled_cards.2026-04-06'))->firstWhere('id', $ticket->id));
        $this->assertEmpty($response->json('data.unscheduled_cards.2026-04-07'));
    }

    public function test_personal_excludes_cancelled_unscheduled_ticket(): void
    {
        $this->createUnscheduledTicket(
            $this->employee,
        ...
        $response->assertStatus(200);
        $this->assertEmpty($response->json('data.unscheduled_cards'));
    }

    public function test_personal_ticket_with_shift_backed_project_is_not_unscheduled(): void
    {
        $this->createTicket(
            $this->employee,
            status: 'in_progress',
        ...
        $morningCard = collect($response->json('data.day_cards.2026-04-05'))->firstWhere('shift.id', $this->morning->id);
        $this->assertSame(1, $morningCard['ticket_count']);
    }
    ...
    public function test_team_lists_unscheduled_ticket_per_account(): void
    {
        $ticket = $this->createUnscheduledTicket(
            $this->employee,
            status: 'in_progress',
        ...
        $response->assertStatus(200)
            ->assertJsonPath("data.unscheduled_cards_by_account.{$this->employee->id}.2026-04-05.0.id", $ticket->id)
            ->assertJsonPath("data.unscheduled_cards_by_account.{$this->employee->id}.2026-04-05.0.subject", 'Dịch vụ sơn tường');
    }
